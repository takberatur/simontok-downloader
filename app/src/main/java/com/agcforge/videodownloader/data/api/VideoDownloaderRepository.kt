...
class VideoDownloaderRepository {
    ...
    suspend fun sendNotifError(request: Map<String, String>): Result<Any> {
        return try {
            val response = api.sendNotifError(request)
            if (response.isSuccessful && response.body()?.success == true) {
                response.body()?.data?.let {
                    Result.success(it)
                } ?: Result.failure(Exception("No data returned"))
            } else {
				Result.failure(Exception(apiError(response.body(), "Failed to send notif error")))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
	...
	suspend fun upsertSubscription(request: SubscriptionUpsertRequest): Result<Subscription> {
		return try {
			val response = api.upsertSubscription(request)
			if (response.isSuccessful && response.body()?.success == true) {
				response.body()?.data?.let { Result.success(it) }
					?: Result.failure(Exception("No data returned"))
			} else {
				Result.failure(Exception(apiError(response.body(), "Failed to save subscription")))
			}
		} catch (e: Exception) {
			Result.failure(e)
		}
	...
	suspend fun getCurrentSubscription(): Result<Subscription?> {
		return try {
			...
			if (response.isSuccessful && response.body()?.success == true) {
				...
				Result.failure(Exception(apiError(response.body(), "Failed to get subscription")))
			}
		} catch (e: Exception) {
			Result.failure(e)
		}
